					...
					<article class="group cursor-pointer {index < 2 ? 'md:col-span-1 lg:col-span-2' : ''}">
						<a href={formatPermalink(post.date, post.slug.current)}>
							<div class="overflow-hidden rounded-2xl bg-white shadow-lg transition-all duration-300 hover:-translate-y-1 hover:shadow-xl">
								<div class="relative {index < 2 ? 'h-64' : 'h-48'} overflow-hidden bg-gradient-to-br from-neutral-100 to-neutral-200">
									<img
								...
								<div class="p-6">
									...
									<h3 class="mb-3 line-clamp-2 {index < 2 ? 'text-lg' : 'text-base'} font-bold text-neutral-900 transition-colors group-hover:text-mocha-600">
										{post.title}
									</h3>
